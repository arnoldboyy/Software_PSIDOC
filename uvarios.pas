...
function editarPaciente(Connection: TMySQL80Connection;
  nombre, licenciatura, grupo: string; matricula, edad: Integer;
  fechaNac, telefono, correo, lugarOrigen, estadoCivil, sexo: string): Boolean;
function editarEnfermedadesFisicas(Connection: TMySQL80Connection; matricula: Integer; enfermedad: string): Boolean;
function editarPerfil(Connection: TMySQL80Connection; matricula: Integer;
  antecedentesAdicciones, enojadoCuando, felizCuando, tristeCuando,
  miedoCuando, desagradoCuando, observacionCuestionario: string): Boolean;
...
begin
  ...
  try
    Query.DataBase := Connection;
    Query.SQL.Text := 'UPDATE paciente ' +
                      'SET nombre = :nombre, licenciatura = :licenciatura, grupo = :grupo, edad = :edad, fechaNac = :fechaNac, ' +
                      'telefono = :telefono, correo = :correo, lugarOrigen = :lugarOrigen, estadoCivil = :estadoCivil, sexo = :sexo ' +
...
begin
  ...
  try
    Query.DataBase := Connection;
    Query.SQL.Text := 'UPDATE enfermedadesFisicas SET enfermedad = :enfermedad ' +
                      'WHERE matriculaPaciente = :matriculaPaciente';
    Query.Params.ParamByName('matriculaPaciente').AsInteger := matricula;
    Query.Params.ParamByName('enfermedad').AsString := enfermedad;
    Query.ExecSQL;
    Connection.Transaction.Commit;
...
begin
  ...
  try
    Query.DataBase := Connection;
    Query.SQL.Text := 'UPDATE perfil SET antecedentesAdicciones = :antecedentesAdicciones, ' +
                      'enojadoCuando = :enojadoCuando, felizCuando = :felizCuando, ' +
                      'tristeCuando = :tristeCuando, miedoCuando = :miedoCuando, ' +
                      'desagradoCuando = :desagradoCuando, ' +
                      'observacionCuestionario = :observacionCuestionario ' +
